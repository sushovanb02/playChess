...
let _settings;//private variable which stores information about the player and state of the game

let options = [],//array of options for every piece on actual board
...
that.movePieceTo = function(stg){
	...
	if(stg.special != null){
		...
		if(stg.special.type=="en"){
			...
			//move that rook
			let rook = CHESSAPP.Analyzer.pieceExists({pieces:that.pieces, x:stg.special.rookx, y:stg.special.rooky});
...
that.onlineMove = function(data){
	...
	let pieceMoved = CHESSAPP.Analyzer.pieceExists({pieces: that.pieces, x: data.pieceX, y: data.pieceY});
	if(pieceMoved){
		if(data.promotion){
